...
%DIF DEL BSS7782_prev.tex   Mon Mar 25 10:38:59 2019
%DIF ADD BSS7782.tex        Mon Apr  8 11:03:34 2019
...
We propose \DIFdelbegin \DIFdel{a formation mechanism for }\DIFdelend \DIFaddbegin \DIFadd{that }\DIFaddend twin blue stragglers (BSs) in compact binaries \DIFdelbegin \DIFdel{that involves }\DIFdelend \DIFaddbegin \DIFadd{evolve
through a phase of }\DIFaddend mass transfer from \DIFdelbegin \DIFdel{an evolved }\DIFdelend \DIFaddbegin \DIFadd{a giant }\DIFaddend outer tertiary companion
on to the inner binary\DIFdelbegin \DIFdel{via a circumbinary disk}\DIFdelend .  We apply this scenario to the \DIFdelbegin \DIFdel{observed double BS-binary WOCS~}\DIFdelend \DIFaddbegin \DIFadd{twin-BS binary
WOCS~ID~}\DIFaddend 7782 in the old open cluster NGC~\DIFdelbegin \DIFdel{188, and show that its observed properties
can be reproduced within the context of the proposed model.
...
= 1.1$ }\DIFdelend \DIFaddbegin \DIFadd{188.  This binary has two
comparable-mass main-sequence stars in an almost circular ($e \aplt
0.1$) orbit of $\aplt 10$\,days.  Our theoretical arguments are
supported by simulations of an inner binary that accretes from an outer
Roche-lobe overfilling star using the Astrophysical Multipurpose
...
stragglers.  In order to acquire a relatively stable phase of mass
...
when starting with a $m_{\rm 3} = 1.4$ }\DIFaddend M$_{\odot}$ \DIFdelbegin \DIFdel{and $m_{\rm 2} = 0.7$ }\DIFdelend \DIFaddbegin \DIFadd{outer star in an
220 to 1100~day orbit around an inner binary composed of an
1.1\,\MSun\, primary and a $m_{\rm 2}=0.7$ }\DIFaddend to $0.9$M$_{\odot}$
\DIFdelbegin \DIFdel{and an outer
tertiary mass of $m_{\rm 3} = 1.4$ M$_{\odot}$.  (2)
After Roche-lobe overflow, the outer star turns into a white dwarf
(WD) of mass 0.43 }\DIFdelend \DIFaddbegin \DIFadd{secondary star in an 8.6 }\DIFaddend to \DIFdelbegin \DIFdel{0.54}\DIFdelend \DIFaddbegin \DIFadd{24}\DIFaddend \,\DIFdelbegin \DIFdel{\MSun. There is a correlation between the
mass of this WD and the outer orbital period: more massive WDs will be
on wider orbits. (3) The rotational axes of both BSs will be aligned
with each other and the orbital plane of the
outer tertiary WD. (4)
...
accretor should, therefore, be enriched more effectively by the accreted material.  As a result, one of the BSs will appear to be more
enriched by either He, C and O or by s-process elements, depending on
whether the donor started to overflow its Roche lobe on, respectively,
the red giant or early asymptotic giant branch.  (5) Relative to old dense clusters with high-velocity dispersions, twin BSs in close
binaries formed from the proposed mechanism should be more frequent in the Galactic field and younger open clusters with ages $\lesssim$ 4-6
Gyr, since then the donor will have a radiative envelope. (6) }\DIFdelend \DIFaddbegin \DIFadd{day orbit.  Based on our simulations
and theoretical arguments we predict that twin BSs that
formed through mass transfer from a Roche-lobe over-filling outer tertiary
are generally comparable in mass and have aligned spins, which are in turn 
aligned with }\DIFaddend the orbit of the \DIFdelbegin \DIFdel{binary BS will
have a small semi-major axis (typically
$\aplt 0.3$\,au) and be close to circular ($e \aplt 0.1$)}\DIFdelend \DIFaddbegin \DIFadd{tertiary white-dwarf star.  If the two
inner stars were initially unequal in mass the less massive star will
have accreted more material from the tertiary star, and should therefore be 
more enhanced in CNO-processed material}\DIFaddend .
...
not accreted to any of the two inner stars.  \DIFdelbegin \DIFdel{This value is
hard to estimate from the simulations , but an accretion efficiency of
$\apgt 0.8$ is necessary to make the scenario feasible. Over the }\DIFdelend \DIFaddbegin \DIFadd{In our simulations the
...
observed in WOCS~ID~7782.  Over the relatively short }\DIFaddend time scale for
which we performed the calculations \DIFdelbegin \DIFdel{, }\DIFdelend this efficiency is \DIFdelbegin \DIFdel{reached}\DIFdelend \DIFaddbegin \DIFadd{achieved}\DIFaddend , but
...
observed twin BS \DIFaddbegin \DIFadd{WOCS }\DIFaddend 7782. In our simulations the eccentricity of the
...
\citet{2014MNRAS.438.1909D}), \DIFdelbegin \DIFdel{which could then subsequently spin
  down }\DIFdelend \DIFaddbegin \DIFadd{but the twin blue stragglers in
WOCS~ID~7782 are not observed to be spinning that fast
\mbox{%DIFAUXCMD
\cite{2018ApJ...869L..29L}}%DIFAUXCMD
.  Rapidly spinning stars may slow }\DIFaddend due to,
for example, magnetic braking, bringing them closer to the actually
observed spin rates \DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{2018arXiv181202181L}}%DIFAUXCMD
.
  }

\DIFaddend %% But by this time both stars have accreted
...
%%}.                                                                            
  ...
  The mass ratio of the inner binary, therefore grows to unity.  
    ...
    We further validated this statement by performing an additional
    series of calculations in which we vary the mass of the tertiary
    star in the initial triple from 0.5\,\MSun\, to 0.7\,\MSun\, and
    0.9\,\MSun. In fig.\,\ref{fig:mass_ratio_evolution_for_14MSun} we
    present the evolution of the normalized mass ratio in these
    binaries. \DIFdelbegin \DIFdel{with
    }\DIFdelend \DIFaddbegin \DIFadd{With }\DIFaddend these calculations we demonstrate that a low-mass
    ratio initially tends to evolve towards an equal mass ratio.
  ...
  \caption{Evolution of the mass ratio for initial triples with an
    ...
    100\,\RSun. \DIFdelbeginFL \DIFdelFL{the }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{The }\DIFaddendFL companion masses are $m_2= 0.9$\,\MSun\, and the
    mass of the tertiary \DIFdelbeginFL \DIFdelFL{is }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{are }\DIFaddendFL indicated in the legend. The dotted black
    line indicates the required mass-ratio evolution in order to
    eventually reach an equal mass-ratio \DIFdelbeginFL \DIFdelFL{blue-straggler }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{blue straggler }\DIFaddendFL binary.
...
\DIFdel{As a consequence of the above, the initially lower mass MS star
  ...
  Note that part of the mass liberated from the triple system
    ...
    J122859.93+104032.9 \mbox{%DIFAUXCMD
\citep{Manseretal2019}}%DIFAUXCMD
.  
}%DIFDELCMD < 
...
\DIFdel{We expect twin BSs in compact binaries formed from the mechanism
  ...
  \mbox{%DIFAUXCMD
\citep[e.g.][]{1991ApJS...76...55I,2009pfer.book.....M}}%DIFAUXCMD
, and a
...
\DIFdelbegin \DIFdel{Such a close }\DIFdelend \DIFaddbegin \DIFadd{As discussed in Section~\ref{intro}, }\DIFaddend BS-WD \DIFdelbegin \DIFdel{binary would be hard to explain in another way.
}\DIFdelend \DIFaddbegin \DIFadd{binaries are predicted to be the products of the binary mass transfer hypothesis (ignoring a common envelope phase) for BS formation. However, this mechanism tends to predict wide orbits, which is consistent with the observed BS-WD systems in NGC 188.  
}\DIFaddend %% These ``imposter'' BS-WD binaries would in principle mimic what is
%% expected theoretically for BSs formed from mass transfer in binary
%% stars.
If\DIFaddbegin \DIFadd{, on the other hand, }\DIFaddend such a dynamical instability engages relatively late in the
