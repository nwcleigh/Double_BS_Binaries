...
our scenario to the observed double blue straggler system Binary 7782
in the old open cluster NGC 188, and show that its observed properties
are naturally reproduced within the context our proposed model.
Within the context of the hypothetical formation mechanism proposed
here, the presented work predicts the following properties for the
post-mass transfer double BS tertiary: (1) For the outer tertiary
orbit, the initial orbital period should lie between 220 days
$\lesssim$ P$_{\rm out}$ $\lesssim$ 1100 days \simon{Do you think that
  the initial binary could be as large as 1400 days?}, assuming
initial masses for the inner binary components of m$_{\rm 1} =$ 1.1
M$_{\odot}$ and m$_{\rm 2} =$ 0.9 M$_{\odot}$ and an outer tertiary
mass of m$_{\rm 3} = 1.4$ M$_{\odot}$.  After Roche-lobe overflow, the
outer star turns into a white dwarf (WD). There is a correlation
bewteen the mass of this WD and the outer orbital period in the sense
that a more massive white dwarfs will be on a wider orbit.  (3) The
rotational axes of the BSs in the inner binary should be aligned with
each other and the orbital plane of the outer tertiary WD. (4) The BSs
in the inner binary should have roughly equal masses, independent of
their initial masses.  This predicts that the initially lower mass MS
star should accrete the most, and should hence be enriched more
effectively by the accreted material.  This could be observable in the
surface layers of a radiative star (i.e., He, C and O enrichment if
the donor is an RGB star, and/or s-process elements if the donor is an
AGB star). (5) Twin BSs in compact binaries formed from the proposed
mechanism should be more frequent in younger clusters \simon{WHy not
  in the field?} with ages $\lesssim$ 4-6 Gyr, since the donor will
have a radiative envelope. (6) the orbital of the binary BS will be
tight (typically $\aplt 0.3$\,au) and close to circular ($e \aplt
0.2$).
...
binaries \cite{+++} of hierarchical triple star systems induced by Lidov-Kozai
...
section~\ref{dyn}, we constrain the range of initial (i.e., pre-mass
...
radius of the sink particles we adopt $2 R_\odot$ for both stars.
...
and $m_{\rm 3} = 1.2$ M$_{\rm \odot}$ for the tertiary star.  We
evolve the 1.2\,\MSun\, star using the MESA stellar evolution code
\cite{2011ApJS..192....3P} to a radius of about 1\,au.  By this time
the star has lost 0.13\,\MSun\, via a stellar wind.  We turned the
stellar evolution model, structure and temperature composition, into a
smoothed-particles representation using the module {\tt
  StellarModelInSPH} in AMUSE (see chapter 4 in \cite{AMUSE}).  We
replace the stellar core by a point mass to precent that all the
resolution will be in the central portion of the star instead of the
envelope. In a following step we relax the star using the
hydrodynamics solver. This relaxation process is realized in 100 steps
during which we reduce the velocity dispersion of individual SPH
particles to a glasseas structure \citep[see, for example, \S\,3.3 on
  page 40 in][]{1994astro.ph.10043W}.


With these parameters the orbital separation of the outer binary
becomes 3.86\,au, and the mass loss has driven a slight eccentricity
deviating from an initially circular orbit to about 0.0082 at the
moment of Roche-lobe overflow. We choose a relative initial
inclination of $i = 9^\circ$ between the inner and the outer orbits,
since observed stellar triples composed of low-mass stars have been
observed to favor co-planar configurations \citep{moe18}.
%NL: What are the initial orbital separations?  Why this choice of initial inclination? - I added a sentence and reference to justify it.
...
\caption{Evolution of the masses (normalized to those at the moment of
  RLOF) for each of the three stars in the simulated triple as a function of
...
the inner binary.  In the simulations we measure ${\dln a \over dt}
\simeq -0.0843$\,Myr$^{-1}$.
