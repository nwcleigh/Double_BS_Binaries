...
ages $\lesssim$ 4-6 Gyr, since the donor will have radiative envelope.
...
double BS binary, called Binary 7782.  Specifically, \citet{mathieu09}
...
have long orbital periods. Dynamically, it difficult to form a
...
So, how did Binary 7782 form?  MENTION BIMODAL P-E DISTRIBUTION, AND
HOW THIS COULD BE SUGGESTIVE OF A TRIPLE ORIGIN FOR SOME OF THE SUBSET
OF OBSERVED BSs.
...
observed for low-mass tertiaries \cite{2010yCat..73890925T} \simon{I
  ...
  references}.  This initial configuration for our assumed formation
...
and is transfers mass to the inner binary.  The mass transfer stream
...
defined by the Roche lobe is given by \citep{eggleton83}.
\simon{Do we really need this eq?}
\begin{equation}
\label{eqn:roche}
R_{\rm L} = \frac{0.49q^{2/3}}{0.6q^{2/3} + {\rm ln}(1 + q^{1/3})}a_{\rm out},
\end{equation}
where the mass ratio q is defined as q $=$ m$_{\rm 3}$/(m$_{\rm 1} +$
m$_{\rm 2}$).  Combining Equation~\ref{eqn:roche} with
...
which we require a$_{\rm c} >$ a$_{\rm in}$, \del{after folding in all
  constraints from the requirements for dynamical stability}\simon{can
  we not simply state the constraints?}, an outer tertiary that is
...
to a contact state for the inner binary, assuming \simon{This looks
  weird:} R$_{\rm 1} =$ R$_{\rm 2} =$ 1 R$_{\odot}$, lies outside the
range of plotted values for P$_{\rm in}$, since it does not contribute
...
\caption{Parameter space in the P$_{\rm out}$-P$_{\rm in}$-plane
  ...
  namely q $=$ 0.1, 0.7, 1, 2 and 10 \simon{before or after mass
    ...
    10?} We assume initial component masses of m$_{\rm 1}$ = 1.1
...
\caption{Orbital separation as a function of the mass of the
  ...
  first giant branch (thick curve) at at the AGB (thin curve). The
  blue curve the orbital separation at the moment this giant fills its
